...
class ResNet_lcs(nn.Module):

    def __init__(self, block, layers, num_classes=17):
        self.inplanes = 64
        super(ResNet_lcs, self).__init__()
        self.gn1 =nn.BatchNorm2d(10)
        self.conv1 = nn.Conv2d(10, 64, kernel_size=3, stride=1, padding=3,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
def resnet18(pretrained=False, **kwargs):
    ...
    return model
def resnet18_lcs( **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_lcs(BasicBlock, [2, 2, 2], **kwargs)
    return model

def resnet34_lcs( **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_lcs(BasicBlock, [3, 4, 6], **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    # model = resnet50_cat3(pretrained=True)
    model = resnet18_lcs()
    # print model
    # print model
    ...
    # #
    # # #
    x = torch.FloatTensor(3,10,32,32).zero_()
